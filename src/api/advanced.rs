...
impl Client {
    ...
    pub async fn get_user_advanced_with_id(&self, user_id: u32) -> Result<AdvancedResponse, Error>{
        Ok(self.get_user_advanced_with_id_option(Some(user_id)).await?)
    }

    pub async fn get_user_advanced(&self) -> Result<AdvancedResponse, Error>{
        Ok(self.get_user_advanced_with_id_option(None).await?)
    }

    async fn get_user_advanced_with_id_option(&self, user_id: Option<u32>) -> Result<AdvancedResponse, Error>{
